...
const aiScene* AiModel::loadModel(const char* fileName)
{
...
aiVector3D AiModel::getRootPos()
{
	return model->mRootNode->mTransformation * model->mMeshes[0]->mVertices[0];
}

AiModel::AiModel(const char* filename, bool iA, bool uB)
{
	isAnimation = iA;
	useBones = uB;
...
void AiModel::setSecsPerTick(float spt)
{
	secsPerTick = spt;
}

void AiModel::updateVerts()
{
	aiMesh* mesh;
	aiBone* bone;
...
void AiModel::updateAnimation(unsigned int tick)
{
...
void AiModel::render() const
{
	render(model, model->mRootNode);
}

void AiModel::releaseResources() const
{
	aiReleaseImport(model);
}

void AiModel::render(const aiScene* sc, const aiNode* nd) const
{
